...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	protected firstLearnsetid(speciesid: ID) {
		...
		if (this.formatType === 'championsgen9lsets') table = table['championsgen9learnsets'];
	...
	getTier(pokemon: Dex.Species) {
		...
		const tableKey = this.formatType === 'doubles' ? `gen${gen}doubles` :
			...
			this.formatType === 'championsgen9lsets' ? `championsgen9learnsets` :
	...
	getBaseResults() {
		...
		const regionBornLegality = dex.gen >= 6 &&
			(/^battle(spot|stadium|festival)/.test(format) || format.startsWith('bss') ||
				...
				(dex.gen === 9 && !this.formatType?.includes('natdex') && this.formatType !== 'championsgen9lsets'));
		...
		if (this.formatType === 'championsgen9lsets') lsetTable = lsetTable['championsgen9learnsets'];
